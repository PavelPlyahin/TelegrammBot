...
bot = telebot.TeleBot(
    '')  # Создаётся объект бота с помощью метода TeleBot,
...
@bot.message_handler(commands=['start']) # функция-обработчик, которая будет вызвана при получении команды /start от
# пользователя.
def start(message):
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)  # Создаем клавиатуру, меняем размер кнопок в зависимости
    # от кол-ва созданных кнопок.
...
@bot.message_handler(func=lambda message: True) #Обработчик сообщений, который будет вызываться для всех входящих сообщений.
